...
include("LPcubature.jl")
  ...
  gaussnodes5,gaussweights5 = gausstensor(4,5)
  gaussnodes7,gaussweights7 = gausstensor(4,7)

  # scale formula to [1,2]^4
  nodes5 = ones(size(nodes5,1),size(nodes5,2)) + nodes5
  nodes7 = ones(size(nodes7,1),size(nodes7,2)) + nodes7

  gaussnodes5 = ones(size(gaussnodes5,1),size(gaussnodes5,2)) + gaussnodes5
  gaussnodes7 = ones(size(gaussnodes7,1),size(gaussnodes7,2)) + gaussnodes7
  ...
  analytic = zeros(length(testfunctions))

  for i in 1:length(testfunctions)
    println("integrating $(testfunctions[i](a1,a2,a3,a4))");
    analytic[i]= float(eval(parse("integrate($(testfunctions[i](a1,a2,a3,a4)),"*"(a1,1,2),(a2,1,2),(a3,1,2),(a4,1,2))")))
  end
  ...
  tensorquadf5 = zeros(length(testfunctions))
  tensorquadf7 = zeros(length(testfunctions))
  ...
  tensorquadf5_tmp = [[testfunctions[i](gaussnodes5[k,:]...) for k in 1:size(gaussnodes5,1)]'gaussweights5 for i in 1:length(testfunctions)]
  tensorquadf7_tmp = [[testfunctions[i](gaussnodes7[k,:]...) for k in 1:size(gaussnodes7,1)]'gaussweights7 for i in 1:length(testfunctions)]
  ...
  for i in 1:length(tensorquadf5_tmp)
    tensorquadf5[i] = tensorquadf5_tmp[i][1]
    tensorquadf7[i] = tensorquadf7_tmp[i][1]
  end
  ...
  tensorrelerror5 = vec([abs(analytic[i]-tensorquadf5[i])/abs(analytic[i]) for i in 1:length(testfunctions)])
  tensorrelerror7 = vec([abs(analytic[i]-tensorquadf7[i])/abs(analytic[i]) for i in 1:length(testfunctions)])
  ...
  for i in 1:length(testfunctions)
    ...
    println("Tensor Gauss relative error: $(tensorrelerror5[i])\n")
  end
  println("\nnumber of nodes: Full Gauss 69, Symmetry Rule $(length(weights5)), Tensor Gauss $(length(gaussweights5))")
  ...
  for i in 1:length(testfunctions)
    ...
    println("Tensor Gauss relative error: $(tensorrelerror7[i])\n")

  end
  println("\nnumber of nodes: Full Gauss 159, Symmetry Rule $(length(weights7)), Tensor Gauss $(length(gaussweights7)) \n")
